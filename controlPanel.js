...
for (let i = 0; i < buscarItem.length; i++) {
    ...
    addn.innerHTML = "<tr><td>"+buscarItem[i].nombre+"</td><td>"+buscarItem[i].telefono+"</td><td>"+buscarItem[i].email+"</td><td>"+buscarItem[i].mesa+"</td></tr>"+"</td><td>"+buscarItem[i].hora+"</td></tr>"+"</td><td>"+buscarItem[i].nota+"</td></tr>"+`<button id="cancelar${[i]}">cancelar</button>`;
    ...
    document.querySelector(`#cancelar${[i]}`).onclick = function (e) {
        ...
        // const clientesR = JSON.parse(clientesT)
        // clientesR.shift(buscarItem[i])
